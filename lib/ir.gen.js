...
irGenerator = function() {
	...
	this.parse = function (astNode, inputs, undefined) {
		if (helper.isArray(astNode)) { // Parse as array of parsed instances
			var arr=[self.parse(astNode[0], inputs)];
			for (var i=1;i<astNode.length;i++) {
				arr.push(self.parse(astNode[i], inputs));
			}
			return arr;
		}
		if (helper.isString(astNode) && astNode) {
		...
		switch (astNode.type) {
			case "Let":
				...
				var letInputs = self.parse(astNode.deflist, inputs);
				color.assign(letInputs);
				let.addNodes(letInputs);
				newInputs = self.addInputs(inputs, letInputs)
			...
			case "If":
				...
				var thenbranch = self.parse(astNode.then, inputs);
				var elsebranch = new node.virtualComplex();
				elsebranch.addNodes(self.parse(astNode.else, inputs));
				var elseif = self.parse(astNode.elseif, inputs);
	...
	this.addInputs = function (inputs, nodes) {
		if (helper.isArray(nodes)) {
			for (var i = 0; i < nodes.length; i++) {
				var tp = new port.typedName(nodes[i].outPorts[0].name, type.simple("auto"));
				tp.color = nodes[i].outPorts[0].color;
				inputs.push(tp);
			}
		} else {
			var tp = new port.typedName(nodes.outPorts[0].name, type.simple("auto"));
			tp.color = nodes.outPorts[0].color;
			inputs.push(tp);
		}
		return inputs;
	}
	...
	this.connectOutput = function (cNode) {
		var outPortNum = 0;
        ...
        if (outPortNum != cNode.outPorts.length)
        	throw "Output parameter list mismatch " + outPortNum + " instead of " + cNode.outPorts.length
	}
